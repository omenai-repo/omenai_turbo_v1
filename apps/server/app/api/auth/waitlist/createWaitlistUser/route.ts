...
export const POST = withRateLimit(strictRateLimit)(async function POST(req: Request) {
    try {
        ...
        if (!email || !name || !entity)  throw new BadRequestError("Invalid parameters provided");

        const waitlistUserExists = await Waitlist.exists({email, entity, name})

        if (waitlistUserExists) throw new ConflictError("User previously added to wait list.");
        ...
        if (!createWaitlistUser) throw new ServerError("An error occured while adding you to our waitlist, please try again or contact support")
