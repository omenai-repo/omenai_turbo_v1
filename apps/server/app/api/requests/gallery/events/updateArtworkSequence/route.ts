...
import z from "zod";
import { updateArtworkSequence } from "../../../../services/gallery/events/updateArtworkSequence.service";
...
export const PATCH = withRateLimitHighlightAndCsrf(config)(async function PATCH(
  ...
  try {
    ...
    const { eventId, galleryId, newSequence } =
      eventMetricsAnalyticsSchema.parse(body);

    if (!eventId || !galleryId || !newSequence) {
    ...
    const result = await updateArtworkSequence(eventId, galleryId, newSequence);
