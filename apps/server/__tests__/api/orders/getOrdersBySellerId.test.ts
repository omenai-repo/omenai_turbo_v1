...
const mockOrders = [
  { order_id: "order-1", seller_details: { id: "seller-123" } },
  { order_id: "order-2", seller_details: { id: "seller-123" } },
];

function makeRequest(id?: string) {
  const url = id
    ? `http://localhost/api/orders/getOrdersBySellerId?id=${id}`
    : "http://localhost/api/orders/getOrdersBySellerId";
  const req = new Request(url, { method: "GET" }) as any;
  req.nextUrl = new URL(url);
  return req;
}

describe("GET /api/orders/getOrdersBySellerId", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 200 with orders for seller", async () => {
    ...
    expect(CreateOrder.find).toHaveBeenCalledWith({ "seller_details.id": "seller-123" });
  });
