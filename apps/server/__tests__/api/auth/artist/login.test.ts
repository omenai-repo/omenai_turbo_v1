  ...
  describe("web login", () => {
    it("returns 200 with session data for valid credentials", async () => {
      ...
      const response = await POST(
        makeRequest({ email: "artist@example.com", password: "password123" }),
      );
      ...
      expect(body.data.email).toBe("artist@example.com");
      expect(body.data.role).toBe("artist");
    });
    ...
    it("returns 409 when artist is not found", async () => {
      mockFindOne(null);
    ...
    it("returns 409 when password does not match", async () => {
      ...
      const response = await POST(
        makeRequest({ email: "artist@example.com", password: "wrongpassword" }),
      );
      const body = await response.json();
    ...
    it("returns 400 when password is missing", async () => {
      const response = await POST(
        makeRequest({ email: "artist@example.com" }),
      );
      const body = await response.json();
  ...
  describe("mobile login", () => {
    ...
    it("updates device push token when device_push_token is provided", async () => {
      ...
      expect(DeviceManagement.updateOne).toHaveBeenCalledWith(
        { auth_id: mockArtist.artist_id },
        { $setOnInsert: { device_push_token: "device-token-xyz" } },
        { upsert: true },
      );
    });
