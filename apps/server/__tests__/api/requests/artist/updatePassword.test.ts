...
vi.mock("@omenai/shared-lib/hash/hashPassword", () => ({
  hashPassword: vi.fn().mockResolvedValue("$2b$10$hashed_new_password"),
}));
...
import { POST } from "../../../../app/api/requests/artist/updatePassword/route";
import { AccountArtist } from "@omenai/shared-models/models/auth/ArtistSchema";
import { VerificationCodes } from "@omenai/shared-models/models/auth/verification/codeTimeoutSchema";
import bcrypt from "bcrypt";

function makeRequest(body: object): Request {
  return new Request("http://localhost/api/requests/artist/updatePassword", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
const validBody = { id: "artist-123", password: "NewPass123!", code: "abc1234" };

describe("POST /api/requests/artist/updatePassword", () => {
  ...
  it("returns 500 when account is not found", async () => {
    vi.mocked(AccountArtist.findOne).mockReturnValue(null as any);
  ...
  it("returns 409 when verification code is invalid", async () => {
    vi.mocked(AccountArtist.findOne).mockReturnValue(mockAccount as any);
    vi.mocked(VerificationCodes.findOne).mockResolvedValue(null as any);
  ...
  it("returns 409 when new password matches the current password", async () => {
    vi.mocked(AccountArtist.findOne).mockReturnValue(mockAccount as any);
    vi.mocked(VerificationCodes.findOne).mockResolvedValue({ code: "abc1234" } as any);
