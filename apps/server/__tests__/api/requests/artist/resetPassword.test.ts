...
import { POST } from "../../../../app/api/requests/artist/resetPassword/route";
...
const mockAccount = { password: "$2b$10$old_hashed_password" };

describe("POST /api/requests/artist/resetPassword", () => {
  ...
  it("returns 400 when token is invalid", async () => {
    ...
    const response = await POST(makeRequest({ password: "NewPass123!", id: "invalid" }));
    const body = await response.json();
  ...
  it("returns 400 when required fields are missing", async () => {
    const response = await POST(makeRequest({ password: "NewPass123!" }));
    const body = await response.json();
