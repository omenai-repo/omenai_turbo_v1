...
describe("POST /api/requests/gallery/sendPasswordResetLink", () => {
  ...
  it("returns 200 and sends reset link successfully", async () => {
    ...
    const response = await POST(makeRequest({ recoveryEmail: "gallery@example.com" }));
  ...
  it("returns 403 when account is not verified", async () => {
    vi.mocked(AccountGallery.findOne).mockReturnValue({
      exec: vi.fn().mockResolvedValue({ ...mockGallery, verified: false }),
    } as any);
    ...
    expect(body.message).toBe("Please verifiy your account first.");
  });
  ...
  it("returns 403 when a token link already exists", async () => {
    ...
    vi.mocked(VerificationCodes.findOne).mockResolvedValue({ author: "gallery-123" } as any);
