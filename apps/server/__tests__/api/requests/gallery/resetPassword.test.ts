...
import { POST } from "../../../../app/api/requests/gallery/resetPassword/route";
...
function makeRequest(body: object): Request {
  return new Request("http://localhost/api/requests/gallery/resetPassword", {
    method: "POST",
...
describe("POST /api/requests/gallery/resetPassword", () => {
  ...
  it("updates the gallery password hash in the database", async () => {
    vi.mocked(VerificationCodes.findOne).mockReturnValue({
      exec: vi.fn().mockResolvedValue({ author: "gallery-123", code: "tok1234" }),
    } as any);
    vi.mocked(AccountGallery.findOne).mockReturnValue(mockAccount as any);
    ...
    expect(AccountGallery.updateOne).toHaveBeenCalledWith(
      ...
      { password: "$2b$10$hashed_new" },
    );
  });

  it("deletes the verification token after a successful password reset", async () => {
    vi.mocked(VerificationCodes.findOne).mockReturnValue({
      exec: vi.fn().mockResolvedValue({ author: "gallery-123", code: "tok1234" }),
    } as any);
    vi.mocked(AccountGallery.findOne).mockReturnValue(mockAccount as any);
