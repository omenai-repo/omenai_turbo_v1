...
export default function CarouselItemSelect({
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center h-[18rem] w-full p-6 bg-white focus:ring ring-1 border-0 ring-dark/10 outline-none focus:outline-none focus:ring-dark transition-all duration-200 ease-in-out rounded-[10px] drop-shadow-lg">
        <div className="w-full">
          ...
          <div className="space-y-3">
            {options.map((option, index) => (
              <label
                key={index}
                className="flex items-center space-x-3 p-3 text-fluid-xxs bg-transparent text-dark rounded-full cursor-pointer transition duration-300 focus-within:ring-2 focus-within:ring-blue-500"
              >
                <input
                  type="radio"
                  name={label}
