...
function SingleOrderCard({ order }: { order: CreateOrderModelTypes }) {
  ...
  const isExclusive =
    order.artwork_data.exclusivity_status?.exclusivity_type === "exclusive" &&
    isArtworkExclusiveDate(new Date(order.createdAt));
            ...
            {isExclusive &&
              ...
              order.status !== "completed" && (
                <span className="text-fluid-xxs text-amber-600 mt-1">
                  This artwork is currently under an exclusivity contract.
                </span>
              )}
          </div>
        </div>
