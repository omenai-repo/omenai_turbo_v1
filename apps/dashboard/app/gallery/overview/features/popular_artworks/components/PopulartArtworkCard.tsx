...
function PopularArtworkCard({ url, title, artist, impressions, rank }: Props) {
  ...
  return (
    <div className="flex items-center justify-between rounded-sm  bg-white p-4 shadow-sm">
      {/* Left */}
      <div className="flex items-center gap-4">
        {/* Rank */}
        <div
          className={`flex h-7 w-7 items-center justify-center rounded-sm  text-xs font-semibold ${styles.badge}`}
        >
          {rank}
        </div>

        {/* Artwork */}
        <div className={`rounded-sm  ring-offset-2 ${styles.ring}`}>
          <Image
            src={image_url}
            alt={title}
            height={56}
            width={56}
            className={`rounded-sm  object-cover ${styles.image}`}
          />
        </div>
