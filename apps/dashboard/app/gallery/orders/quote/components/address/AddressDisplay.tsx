...
export default function AddressDisplay({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -10 }}
      className="p-6 border border-neutral-200 rounded-sm  -lg bg-white shadow-sm flex flex-col gap-4"
    >
      {isRecentlyUpdated && (
        <motion.div
          initial={{ opacity: 0, height: 0 }}
          animate={{ opacity: 1, height: "auto" }}
          className="flex items-start gap-2 bg-green-50 border border-green-100 p-3 rounded-sm  -lg text-green-800 text-sm mb-2"
        >
      ...
      <div className="pt-4 flex flex-col items-start gap-3 mt-2">
        <button
          onClick={onEditClick}
          type="button"
          className="flex items-center gap-2 text-fluid-xs font-normal text-dark bg-neutral-50 hover:bg-neutral-100 px-4 py-2.5 rounded-sm  -lg transition-colors border border-neutral-200"
        >
