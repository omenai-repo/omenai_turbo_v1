...
export default function PackagingSelector({
  ...
  return (
    <div ref={containerRef} className="space-y-6">
      {/* Header */}
      <div className="bg-white border border-slate-200 shadow-sm rounded-sm  p-5 flex flex-col md:flex-row md:items-center justify-between gap-5 transition-all">
        <div className="flex items-start gap-4">
          <div className="shrink-0 mt-0.5">
            <div
              className={`
              w-10 h-10 rounded-sm  flex items-center justify-center border shadow-sm
              ${
          ...
          <div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500 leading-relaxed flex flex-wrap items-center gap-1.5">
                ...
                <span className="inline-flex items-center px-1.5 py-0.5 rounded-sm  bg-slate-100 border border-slate-200 text-slate-700 font-mono font-semibold text-[10px] tracking-wider">
        ...
        <button
          ...
          className="shrink-0 flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-sm  -lg text-xs font-semibold text-slate-700 hover:text-slate-900 transition-all active:scale-95 w-full md:w-auto"
        >
          <ArrowRightLeft
            size={14}
            strokeWidth={2}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {PACKAGING_PRESETS[packagingType].map((preset) => {
          ...
          return (
            <div
              ...
              className={`relative border rounded-sm  overflow-hidden transition-all duration-300 ${isClickable ? "cursor-pointer" : "cursor-not-allowed"} ${cardStyle}`}
            >
              {isRecommended && isCompatible && (
                <div className="absolute top-0 left-0 z-30">
                  <div className="bg-emerald-500 text-white text-[10px] font-bold px-3 py-1 rounded-sm  -br-lg shadow-sm uppercase tracking-wider flex items-center gap-1">
                    <svg
                      className="w-3 h-3"
                      fill="none"
                      viewBox="0 0 24 24"
              ...
              {!isCompatible && (
                <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] flex items-center justify-center z-20">
                  <span className="bg-white text-red-600 text-xs font-bold px-3 py-1.5 rounded-sm  -full shadow-sm border border-red-100 uppercase tracking-wide">
                    Too Small
                  </span>
                </div>
              )}
              {isCompatible && isOversize && !isSelected && (
                <div className="absolute inset-0 bg-white/70 backdrop-blur-[1px] flex items-center justify-center z-20 p-4">
                  <div className="bg-white border border-amber-200 shadow-md rounded-sm  -lg p-3 text-center flex flex-col items-center">
              ...
              <div className="relative h-56 w-full bg-slate-50 flex items-center justify-center overflow-hidden">
                ...
                {isSelected && (
                  <div
                    className={`absolute top-3 right-3 rounded-sm  -full p-1.5 shadow-lg z-10 animate-in zoom-in duration-200 ${isRecommended ? "bg-emerald-500 text-white" : "bg-dark text-white"}`}
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
        ...
        <div
          ...
          className={`flex flex-col items-center col-span-1 sm:col-span-2 w-full justify-center text-center p-6 border rounded-sm  cursor-pointer min-h-[120px] transition-all duration-200 ${isCustom ? "border-dark ring-2 ring-dark ring-offset-2 bg-slate-50 shadow-md" : "border-dashed border-slate-300 hover:bg-slate-50 hover:border-slate-400"}`}
        >
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-sm  -full flex items-center justify-center shadow-sm transition-colors ${isCustom ? "bg-dark text-white border-dark" : "bg-white border border-slate-200 text-slate-400"}`}
            >
              <svg
                fill="none"
                viewBox="0 0 24 24"
      ...
      {isCustom && (
        <div
          ref={customInputRef}
          className="mt-4 p-5 bg-slate-50 rounded-sm  border border-slate-200 animate-in fade-in slide-in-from-top-2 shadow-inner"
        >
          <div className="flex items-center gap-2 mb-4">
            <span className="w-1.5 h-4 bg-dark rounded-sm  -full"></span>
