...
interface ShowArtworksGridProps {
  show: any;
}
type FilterValue = "All" | "Available" | "Sold";

export const ShowArtworksGrid = ({ show }: ShowArtworksGridProps) => {
  ...
  const filteredArtworks = useMemo(() => {
    if (!show.artworks) return [];
    if (activeFilter === "All") return show.artworks;
    return show.artworks.filter((art: any) =>
      activeFilter === "Available" ? art.availability : !art.availability,
    );
  }, [show.artworks, activeFilter]);
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-t border-black pt-8 mb-14">
          <div className="flex flex-col gap-5">
            <div className="flex flex-col gap-2">
              ...
              <h2 className="font-serif text-[clamp(1.8rem,4vw,1.5rem)] font-light text-black leading-tight">
                Featured Works
              </h2>
            </div>
          ...
          <div className="flex items-end gap-8 pb-1">
            ...
            <div className="flex items-baseline gap-1.5">
              ...
              {activeFilter !== "All" && (
                <>
                  <span className="text-neutral-300 text-sm font-sans">/</span>
                  <span className="text-sm font-sans font-light text-neutral-500">
                    {show.artworks?.length}
                  </span>
                </>
              )}
        ...
        <div className="fixed inset-0 z-[100] bg-white flex flex-col">
          ...
          <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-center z-50">
            <span className="font-sans text-[10px] font-medium text-dark uppercase tracking-[0.2em]">
              {show.title} — {filteredArtworks.length} Works
            </span>
            <button
